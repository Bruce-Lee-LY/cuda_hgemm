...
//
// Description: wmma async pg2s hgemm
...
__global__ void wmmaAsyncPg2sKernel(const half *__restrict__ A, const half *__restrict__ B, half *__restrict__ C,
...
size_t initWmmaAsyncPg2s() {
    ...
    HGEMM_CHECK_CUDART_ERROR(
        cudaFuncSetAttribute(wmmaAsyncPg2sKernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_max_size));

    return smem_max_size;
}

void wmmaAsyncPg2s(half *A, half *B, half *C, size_t M, size_t N, size_t K) {
    static size_t smem_max_size = initWmmaAsyncPg2s();

    dim3 block(THREADS_PER_BLOCK);
    dim3 grid(BLOCK_STRIDE, div_ceil(M, BLOCK_ROWS), div_ceil(N, BLOCK_COLS * BLOCK_STRIDE));

    wmmaAsyncPg2sKernel<<<grid, block, smem_max_size>>>(A, B, C, M, N, K);
}
