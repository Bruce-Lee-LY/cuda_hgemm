...
#pragma unroll
        for (size_t i = 0; i < WARP_COL_TILES; ++i) {
            size_t A_smem_idx = smem_load_off + (warp_id / BLOCK_ROW_WARPS) * WARP_ROWS + i * WMMA_M;
            const half *A_tile_ptr = &smem[A_smem_idx][WMMA_K];
...
#pragma unroll
        for (size_t j = 0; j < WARP_ROW_TILES; ++j) {
            size_t B_smem_idx = smem_load_off + B_smem_idx_off + (warp_id % BLOCK_ROW_WARPS) * WARP_COLS + j * WMMA_N;
            const half *B_tile_ptr = &smem[B_smem_idx][WMMA_K];
...
size_t initWmmaAsyncStage2() {
    ...
    HGEMM_CHECK_CUDART_ERROR(
        cudaFuncSetAttribute(wmmaAsyncStage2Kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_max_size));

    return smem_max_size;
}

void wmmaAsyncStage2(half *A, half *B, half *C, size_t M, size_t N, size_t K) {
    static size_t smem_max_size = initWmmaAsyncStage2();

    dim3 block(THREADS_PER_BLOCK);
    dim3 grid(BLOCK_STRIDE, div_ceil(M, BLOCK_ROWS), div_ceil(N, BLOCK_COLS * BLOCK_STRIDE));

    wmmaAsyncStage2Kernel<<<grid, block, smem_max_size>>>(A, B, C, M, N, K);
}
