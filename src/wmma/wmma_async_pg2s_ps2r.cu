...
//
// Description: wmma async pg2s ps2r hgemm
...
__global__ void wmmaAsyncPg2sPs2rKernel(const half *__restrict__ A, const half *__restrict__ B, half *__restrict__ C,
...
#pragma unroll
        for (size_t i = 0; i < WARP_COL_TILES; ++i) {
            size_t A_smem_idx = smem_load_off + (warp_id / BLOCK_ROW_WARPS) * WARP_ROWS + i * WMMA_M;
            const half *A_tile_ptr = &smem[A_smem_idx][0];
...
#pragma unroll
        for (size_t j = 0; j < WARP_ROW_TILES; ++j) {
            size_t B_smem_idx = smem_load_off + B_smem_idx_off + (warp_id % BLOCK_ROW_WARPS) * WARP_COLS + j * WMMA_N;
            const half *B_tile_ptr = &smem[B_smem_idx][0];
...
#pragma unroll
            for (size_t j = 0; j < WARP_ROW_TILES; ++j) {
                size_t B_smem_idx =
                    smem_load_off + B_smem_idx_off + (warp_id % BLOCK_ROW_WARPS) * WARP_COLS + j * WMMA_N;
...
#pragma unroll
    for (size_t i = 0; i < WARP_COL_TILES; ++i) {
        size_t A_smem_idx = smem_store_off + (warp_id / BLOCK_ROW_WARPS) * WARP_ROWS + i * WMMA_M;
        const half *A_tile_ptr = &smem[A_smem_idx][0];
...
#pragma unroll
    for (size_t j = 0; j < WARP_ROW_TILES; ++j) {
        size_t B_smem_idx = smem_store_off + B_smem_idx_off + (warp_id % BLOCK_ROW_WARPS) * WARP_COLS + j * WMMA_N;
        const half *B_tile_ptr = &smem[B_smem_idx][0];
...
size_t initWmmaAsyncPg2sPs2r() {
    ...
    HGEMM_CHECK_CUDART_ERROR(
        cudaFuncSetAttribute(wmmaAsyncPg2sPs2rKernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_max_size));

    return smem_max_size;
}

void wmmaAsyncPg2sPs2r(half *A, half *B, half *C, size_t M, size_t N, size_t K) {
    static size_t smem_max_size = initWmmaAsyncPg2sPs2r();

    dim3 block(THREADS_PER_BLOCK);
    dim3 grid(BLOCK_STRIDE, div_ceil(M, BLOCK_ROWS), div_ceil(N, BLOCK_COLS * BLOCK_STRIDE));

    wmmaAsyncPg2sPs2rKernel<<<grid, block, smem_max_size>>>(A, B, C, M, N, K);
}
